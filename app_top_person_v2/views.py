...
# load data
def load_data_topPerson_v2():
    # read df
    df_topPerson_v2 = pd.read_csv(
        'app_top_person_v2/dataset/省宏專用/PERSON_data.csv')
    # refresh data
    global data # make data global. It can be used everywhere.
    data = {}
    for idx, row in df_topPerson_v2.iterrows():
        data[row['category']] = eval(row['top_keys'])


# Load data first when starting server.
load_data_topPerson_v2()


def home(request):
    return render(request, 'app_top_person_v2/home.html')

# csrf_exempt is used for POST
# 單獨指定這一支程式忽略csrf驗證
@csrf_exempt
def api_get_topPerson_v2(request):
    ...
    chart_data, wf_pairs = get_category_topPerson_v2(cate, topk)
...
def get_category_topPerson_v2(cate, topk):
